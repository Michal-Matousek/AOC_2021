...
flashCycleSync :: Int -> Array (Int,Int) Int -> Int
flashCycleSync i octo = if flashCount == syncCount then i
                                                   else flashCycleSync (i+1) updOcto
...
main = do 
    ...
    let lineIn = [ [0] ++ (map read (map (:[]) l)) ++ [0]  | l <- (lines $ contents) ] :: [[Int]]
        ...
        synCycle = flashCycleSync 1 octo
