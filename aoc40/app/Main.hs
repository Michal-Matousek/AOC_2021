...
doConvolution i algo mapLines boundFill = doConvolution (i-1) algo convMapLines newBoundFill
    where   (szX,szY) = ( length mapLines, length (head mapLines) )
            boundLine = take (szY+4) (repeat boundFill) :: [Int]
            boundMapLines =  [boundLine] ++ [boundLine] ++ [ [boundFill,boundFill] ++ l ++ [boundFill,boundFill] | l<-mapLines] ++ [boundLine] ++ [boundLine] 
...
main = do 
    ...
    let -- INPUT                
        ...
        convMap1 = doConvolution 50 algoArr mapLines 0        
        convMap = convMap1
